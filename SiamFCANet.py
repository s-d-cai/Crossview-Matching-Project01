...
################## Wo/ rotation regression branch ####################
    
###################
class SiamFCANet_Base_Wo_CVUSA(nn.Module):

    def __init__(self, block, layers, num_classes=1024):
        self.inplanes = 64
        self.inplanes_p = 64
        super(SiamFCANet_Base_Wo_CVUSA, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        self.conv1_p = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
    ...
    def forward(self, x_grd, x_sat):
        ...
        return global_grd, global_sat, 1.0
    ...
    def forward_Tri(self, x_A, x_P, x_N):
        ...
        return features_A, features_P, features_N, 1.0
    
##############################################

#### return OR net split version
######### basic block ##############
def SiamFCANet18_Wo_CVUSA( **kwargs):
    """.
    """
    model = SiamFCANet_Base_Wo_CVUSA(FCAMBasicBlock, [2, 2, 2, 2], **kwargs)
    return model

def SiamFCANet34_Wo_CVUSA( **kwargs):
    """.
    """
    model = SiamFCANet_Base_Wo_CVUSA(FCAMBasicBlock, [3, 4, 6, 3], **kwargs)
    return model
