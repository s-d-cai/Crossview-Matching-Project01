...
class SiamFCANet_Base_CVUSA(nn.Module):
    ...
    def forward(self, x_grd, x_sat):
        ...
        angle_ORvec = self.angle_fc8_CVUSA(angle_fc7) # for 1024 lengh vec
        angle_ORvec = self.angle_relu(angle_ORvec)
        angle_ORvec = self.angle_fc9_CVUSA(angle_ORvec) ### vec length for each instance is 2 (sin(angle) and cos(angle))
        #################
